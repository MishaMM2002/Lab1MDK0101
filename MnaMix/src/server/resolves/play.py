...
def create(new: Model) -> int | dict:
    res = dbmanager.execute_query(
        query=f"insert into {Model.__name__} (name, typeID) values(?,?) returning id",
        args=(new.name, new.type_id))
...
def update(id_: int, new: Model) -> None:
    return dbmanager.execute_query(
        query=f"update {Model.__name__} set (name, typeID) = (?,?) where id=(?)",
        args=(new.name, new.type_id, id_))
